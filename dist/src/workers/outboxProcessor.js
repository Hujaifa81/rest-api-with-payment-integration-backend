...
export async function processOutboxEvent(event) {
    if (event.topic !== "CREATE_STRIPE_SESSION")
        return;
    ...
    try {
        ...
        try {
            ...
            console.error("Failed to persist payment/outbox updates in transaction; attempting compensation", dbErr);
        ...
        if (ev && ev.attempts >= 3) {
            ...
            try {
                ...
                console.info("Outbox event exceeded attempts and was marked failed", ev.id, "reason:", reason);
            }
            catch (e) {
